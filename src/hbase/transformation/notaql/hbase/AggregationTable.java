...
public class AggregationTable<R, C, V> implements Table<R, C, List<V>> {
    private Table<R, C, List<V>> table;

    private Table<R, C, List<V>> tableTimeStamp;
    ...
    public void appendTimeStamp(R rowKey, C column, V value) {

        //TODO keep the insertion order correct
        List<V> curVal = this.table.get(rowKey, column);


        if (curVal == null) {
            curVal = new LinkedList<V>();
        }
        curVal.add(value);

        this.table.put(rowKey, column, curVal);
